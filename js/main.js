...
const cart = JSON.parse(localStorage.getItem(`SaveLogin_${login}`)) || [];

checkout.addEventListener('click', () => {
  let modal = document.querySelector(".modal-checkout");
...
function saveCart() {
  localStorage.setItem(`SaveLogin_${login}`, JSON.stringify(cart));
}

function dowloadCart() {
 if (localStorage.getItem(`SaveLogin_${login}`)) {
   const data = JSON.parse(localStorage.getItem(`SaveLogin_${login}`));
   cart.push(...data)
 }
}
//var url = './db/partners.json';
const getData = async function(url) {
    const response = await fetch(url);

    if (!response.ok) {
      throw new Error(`Ошибка по адрессу ${url}, статус ошибка ${response.status}!`)
    }
  ...
  const target = event.target;
    ...
    if (login) {
      ...
      if (restaurant) {
          ...
          const { kitchen, name, stars, } = restaurant.info;
